 ...
 *
 * PUT /api/packages { packages: [{id, label, hari, harga, note}, ...] } —
 * admin-only, bulk-updates package pricing. Only existing package ids
 * (p30/p90/p365) are updated; this endpoint never creates or removes a
 * package tier. Orders already placed keep their own snapshotted `harga`
 * from order time (see api/order.js), so changing a price here never
 * rewrites the amount a student already agreed to pay.
 */
