...
- **A short label ending in ":" immediately followed by an equation gets a `<br>` between them** — e.g. `Komponen sumbu-X:<br>\(F_x = ...\)`. Standing user instruction: without the break, the equation runs on cramped straight after the colon on the same line, especially inside a numbered `.step-item`/`.role-card`. Applies to that specific "label: *then* formula" pattern, not to a colon mid-sentence inside flowing prose (a `<li>`/`<p>` where more text continues naturally after the equation) — don't force a break there, it fragments the sentence instead of fixing a cramped look.
...
- **`.role-cards` flows vertically, one card per row — never a side-by-side grid.** Standing user instruction: a row of narrow columns compresses multi-line steps into an uninviting, hard-to-read shape. Use `.role-cards` > `.role-card` (numbered `<span class="role-n">`) for a general numbered list of items/criteria where each item deserves its own bordered card.
- **Worked examples ("Contoh Soal") with step-by-step solutions use `.step-card`, not `.role-cards`.** `.step-card` is a *single* bordered container; each step is a `.step-item` (numbered `<span class="step-n">`) divided from the next by an internal border, so the whole solution reads as one continuous card rather than several separate ones stacked or gridded. Standing user instruction — do not use `.role-cards`/`.role-card` for Contoh Soal steps. **Use `.step-item`, never `.step-row`** — `.step-row` is a different, pre-existing class (Biologi's mitosis-phase pill toggle, a `display:flex` row of clickable pills) and reusing that name silently inherits its flex layout, which breaks KaTeX's baseline alignment with the label text.
- **Lists — always numbered, never a checkmark/bullet:** any itemized list in the HTML lesson (principles, criteria, steps, characteristics) uses `<ul class="check-list">`. The class name is legacy; its CSS renders numbered markers (`1.`, `2.`, …), not the checkmark the name implies — this was a standing user instruction after an earlier chapter shipped with checkmark bullets. Don't hand-roll a numbered look with inline styles or switch to `<ol>` with browser default numbering; reuse `check-list` as-is so every chapter's lists stay visually consistent.
...
**Diagram sourcing — check the source material for a real diagram before ever hand-drawing one.** A real bug from Ekonomi Bab 2: a circular-flow diagram and a Lorenz-curve diagram were both hand-built as inline SVG, when the supplied source PDF actually contained real diagram images for both (embedded raster images, extractable at full resolution with e.g. PyMuPDF: open the PDF, `page.get_image_info(xrefs=True)` to find embedded images and their bbox, `fitz.Pixmap(doc, xref)` to pull the original at native resolution — no re-rasterizing needed). The hand-drawn recreation wasn't just lower-quality, it was **wrong**: it silently dropped a real connection the source diagram had (an import flow), because freehand recreation from a text description is exactly as reliable as that description was complete. Rule: before building any diagram as inline SVG, check every supplied source PDF/document for an existing image covering the same content (`page.get_images()`/`get_image_info()` on every page, not just skimming the extracted text — a parallel multi-file read can silently truncate a large PDF's content, so re-read individually if a diagram is expected but the first pass showed none). If one exists, extract and use it directly as a static `<img>` (see `.diagram-card` pattern below) — never redraw it from memory of what it showed, and never build separate partial-view SVGs (e.g. "2-sektor", "3-sektor" cutdowns) when only one full diagram was actually supplied; explain the simpler cases in text next to the one real image instead of inventing diagrams for them. This extends the existing "never invent a Sketchfab URL" rule to diagrams in general: a supplied source image is the source of truth, not a description of one.
...
**Per-subject `--accent` colors** (add new ones here when a subject's color is decided, never invent one per-chapter): `ekonomi` → `#ee7d31` (site's orange). `biologi` → `#3fa15c` (matches `subjectsData.biologi.color: 'green'` in `app.js`). `matematika` → `#2f7edb` (matches `subjectsData.matematika.color: 'blue'` and `QUIZ_PDF_ACCENTS.matematika` in `app.js`). `fisika`/`kimia` not yet decided.
...
- **Multiple packages ("Paket 1", "Paket 2", …):** when the user supplies more than one bank soal PDF for the same chapter, give each its own `bab` value (`{bab}-extra-bank-soal-paket2.pdf.{page}.png`, `-paket3`, etc. — `paket1` is just the original `-extra-bank-soal` value, no suffix needed) and its own `.pdf-viewer` block, then switch between them with the existing `[data-tabswitch]`/`.toggle-switch` pattern (see `content-private/matematika/bab2-fungsi-komposisi-invers.html`'s Extra Bank Soal section for the live example) rather than inventing a new picker. Each viewer keeps its own independent prev/next/download state — they're just two ordinary `.pdf-viewer`s, one per `[data-tabpanel]`.
...
- **"Kalkulator Lab" is reserved for genuinely medium/hard, multi-step calculations — not every formula on the page needs one.** A real bug from Ekonomi Bab 2: a "Kalkulator Lab" was built for `Y = C+I+G+(X-M)` (one line of addition/subtraction) and another for Gini classification (a plain range lookup, no arithmetic at all) — both got removed for being basic math dressed up as a lab. The bar is the existing SHU calculator in Bab 1: two independent ratios (your simpanan/total simpanan, your belanja/total belanja) each scaled by an independent percentage, summed — genuine multi-variable, multi-step reasoning a student couldn't do in their head. Before adding a "Kalkulator Lab" for a formula, check: is this more than one arithmetic step, with more than one input relationship? If not, a static worked example on the page teaches the same thing without inflating an interactive component for basic math.
- **When a calculator involves two or more independent proportions/percentages of the same pool (e.g. SHU dibagi jasa modal + jasa usaha + dana cadangan + ...), don't hardcode one as `100 - other`.** A real bug: the SHU calculator computed jasa usaha's percentage as `100 - pctModal`, silently assuming the two categories exhaust the whole SHU — but a koperasi's AD/ART typically carves out several funds (cadangan, dana pengurus, dana pendidikan, dst.) from the same SHU pool, so jasa modal and jasa usaha are independent percentages that don't have to sum to 100%. Model each proportion as its own input, and if there's a natural "remainder" concept, compute and show it (with a validity check if the independent percentages could sum past 100%) rather than deriving one from `100 - other`.

### Sejarah
- Prefer a causal chain over a list of dates — not "1602: VOC didirikan", walk through why (persaingan dagang antar kongsi Belanda) → what changed (monopoli, hak octrooi) → what it led to (eksploitasi, kebangkrutan 1799, wilayah diambil alih negara).
- Distinguish the official/formal version of a policy from what actually happened on the ground where the two diverge (e.g. Cultuurstelsel's stated rules in Staatblad 1834 vs its real-world abuses) — this gap is usually the most exam-relevant and most interesting part.
- A timeline table (`ol.ref-list` in the PDF, or a compact `.compare-table`-style table in HTML for a short one) is the natural structure for chronological content — use it for a real sequence of events, not as a dumping ground for everything.
- Maps and route diagrams earn their place here the way structural diagrams do in Biologi — a voyage/route map genuinely clarifies "who went where, in what order" better than prose. Follow the same diagram-sourcing rule: if a real historical map/route image is supplied, use it directly (`.diagram-card`), don't hand-draw a recreation.
- Name real historical actors, dates, and primary decisions (treaties, edicts, expeditions) precisely — vague summary ("bangsa Eropa datang ke Indonesia") is weaker and less exam-accurate than the specific chain of who/when/why.

### Bahasa Inggris
- Audience is still an Indonesian SMA student, but the target language is English — so unlike every other subject, the worked examples/quiz options/example sentences are themselves in English, while explanations of *why* stay in the same casual Bahasa Indonesia as every other chapter. Don't explain grammar in English; explain it in Indonesian, using English only for the example sentences and quiz content being tested.
- **Every tense/structure gets three things, in this order, never skipped:** (1) **Bentuk (form)** — the (+)/(−)/(?) pattern with the actual formula, e.g. `S + V1 (+s/es untuk he/she/it)`, `S + is/am/are + V-ing`, using a `.law-strip` per pattern (not KaTeX — this is English morphology, not math notation; plain text/HTML is correct here, e.g. `<i>He <b>plays</b> football.</i>` for the example, not `\(V_1\)`). (2) **Fungsi (usage/meaning)** — when this structure is actually used, not just how it's built; a tense with multiple distinct uses (e.g. Simple Present for kebiasaan/general truth/jadwal tetap) gets each use listed separately with its own example, not lumped into one sentence. (3) **Signal words** — the specific adverbs/time expressions that cue this tense (e.g. *always, usually, every day* for Simple Present; *now, right now, at the moment* for Present Continuous) as a `check-list`, since students pattern-match on these heavily in exam questions.
- **Every English example sentence is followed by its Indonesian translation in parentheses**, e.g. `<i>She <b>is cooking</b> dinner right now.</i> (Dia sedang memasak makan malam sekarang.)` — bold the specific verb form/structure being illustrated inside the example, every time, so the pattern is visually scannable down a list of examples.
- **Contrast tables are the single most important tool in this subject.** Any two tenses/structures a student can confuse (Simple Past vs Present Perfect, Present Continuous vs Simple Present, gerund vs to-infinitive after certain verbs) get a `.compare-table` sitting right next to both, built around the actual distinguishing question ("apakah waktunya spesifik disebutkan?", "apakah aksinya masih berhubungan dengan sekarang?") — not just two definitions side by side that leave the reader to spot the difference themselves.
- **Name the exact Indonesian-learner mistake, not a generic "hati-hati."** Indonesian has no verb conjugation or tense marking, so the recurring real errors are specific and predictable: dropping `to be` (`*She happy*` instead of `She is happy`), forgetting third-person `-s/-es` (`*He go*` instead of `He goes`), using base verb after modals with `to` (`*can to go*`), misordering adjectives, confusing `since` (a point in time) with `for` (a duration), using Simple Past time signals with Present Perfect or vice versa. Call these out explicitly as ❌/✅ pairs (see guide's general §2 structure), because these specific patterns are exactly what exam distractors are built from.
- **Irregular verb forms are reference material, not prose** — a full V1-V2-V3 table (or the relevant subset for the chapter) belongs in a `.compare-table`/`ref-table`-style table, not written out as a list of sentences.
- Progression across worked examples still applies (direct form-recognition → fill-in-the-blank transformation → error-correction → exam-style reading-comprehension-style question), same as every other subject.
- Quiz questions should mix: identify-the-correct-form (fill in the blank), spot-the-error, transform-the-sentence (active↔passive, direct↔reported if ever covered, tense-shift), and short reading-context questions — not just "which tense is this," which tests recognition without production.
